...
/* 广播名称 */
static uint8_t sle_local_name[NAME_MAX_LENGTH] = SLE_SDK_SERVER_NAME;
...
static uint16_t sle_set_adv_local_name(uint8_t *adv_data, uint16_t max_len)
{
    ...
    uint8_t local_name_len = (uint8_t)strlen((const char *)local_name);
...
errcode_t sle_uart_server_adv_init(void)
{
    errcode_t ret;
    SleAddr addr;
    unsigned char local_addr[SLE_ADDR_LEN] = SLE_SDK_SERVER_ADDR_INIT;
    addr.type = 0;
    memcpy(addr.addr, local_addr, SLE_ADDR_LEN);
