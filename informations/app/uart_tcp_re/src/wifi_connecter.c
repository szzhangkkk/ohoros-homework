                            ...
                            wifi_sta_config_stru *expected_bss)
{
    ...
    {
        // 打印扫描到的SSID，方便调试
        printf("Scanned SSID: %s\r\n", result[bss_index].ssid);
        
        // 使用strncmp比较字符串，避免strlen的问题
        if (strncmp(expected_ssid, result[bss_index].ssid, WIFI_MAX_SSID_LEN) == 0)
        {
            break;
        }
    }
    ...
    expected_bss->ip_type = DHCP; // IP类型为动态IP
...
errcode_t ConnectToHotspot(char *ssid, char *key)
{
    ...
    if(wifi_register_event_cb(&wifi_event_cb) != ERRCODE_SUCC){
        printf("Register wifievent fail\r\n");
        return ERRCODE_FAIL;
    }
    ...
    // 启用网络接口
    netifapi_netif_set_up(g_iface);

    // 启动DHCP
    if (netifapi_dhcp_start(g_iface) != ERR_OK)
    {
        printf("STA start DHCP fail.\r\n");
        return ERRCODE_FAIL;
    }

    // 等待DHCP分配IP
    uint8_t index = 0;
    for (index = 0; index < WIFI_STA_IP_MAX_GET_TIMES; index++)
    {
        osal_msleep(1000);
        if (netif_is_up(g_iface) && (g_iface->ip_addr.u_addr.ip4.addr != 0))
        {
            break;
        }
    }
    ...
    printf("Connect success with DHCP.\r\n");
    return ERRCODE_SUCC;
}
