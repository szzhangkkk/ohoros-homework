...
#define BLE_SERVER_ADV_LOG "[ble server adv]"
...
uint8_t g_ble_server_adv_name[MAX_NAME_LENGTH] = BLE_SDK_SERVER_NAME;
...
static uint8_t ble_set_adv_name(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ...
    ble_local_name_t adv_local_name_data = {
        .length = (uint8_t)(BLE_ADV_PARAM_DATATYPE_LENGTH + sizeof(g_ble_server_adv_name)),
    ...
    if (memcpy_s((set_adv_data_position+len), (size_t)(max_len-len),
                 g_ble_server_adv_name, sizeof(g_ble_server_adv_name)) != EOK) {
        return 0;
    }
    len = (uint8_t)(len + sizeof(g_ble_server_adv_name));
    return len;
}
...
static uint16_t ble_server_build_adv_data(uint8_t *set_adv_data, uint8_t adv_data_max_len)
{
    uint8_t idx = 0;
...
static uint16_t ble_set_scan_response_data(uint8_t *scan_rsp_data, uint8_t scan_rsp_data_max_len)
{
    ...
    /* set local name */
    scan_rsp_data[idx++] = sizeof(g_ble_server_adv_name) + BLE_GENERAL_BYTE_1;
    scan_rsp_data[idx++] = BLE_ADV_LOCAL_NAME_DATA_TYPE;
    if ((idx + sizeof(g_ble_server_adv_name)) > scan_rsp_data_max_len) {
        return 0;
    }
    if (memcpy_s(&scan_rsp_data[idx], scan_rsp_data_max_len - idx, g_ble_server_adv_name,
                 sizeof(g_ble_server_adv_name)) != EOK) {
        return 0;
    }
    idx += sizeof(g_ble_server_adv_name);
    return idx;
}
...
uint8_t ble_server_set_adv_data(void)
{
    ...
    /* set adv data */
    uint16_t adv_data_len = ble_server_build_adv_data(set_adv_data, EXT_ADV_OR_SCAN_RSP_DATA_LEN);
    ...
    printf("%s ble_server_set_adv_data adv_handle %d, len:%d, data:%s\n",
                BLE_SERVER_ADV_LOG, BTH_GAP_BLE_ADV_HANDLE_DEFAULT, adv_data_len, set_adv_data);
...
uint8_t ble_server_start_adv(void)
{
    ...
    printf("%s ble_server_start_adv adv_id %d\n", BLE_SERVER_ADV_LOG, adv_id);
